...
if pivot_df_clean is not None:
    ...
    with tab_allocation:
        if st.session_state.get("trained", False):
            # Tính toán phân bổ tỷ trọng trung bình trên tập test của LSTM-GRU
            ...
            # Tính các tỷ trọng danh mục đối chứng
            Allo_1 = pd.DataFrame({
                "Asset": top_symbols,
            ...
            with col_al1:
                st.markdown('<div class="card-box" style="height: 100%;">', unsafe_allow_html=True)
                st.markdown('<div class="card-title">Tỷ trọng danh mục LSTM-GRU</div>', unsafe_allow_html=True)
                st.dataframe(results_LSTM_GRU.style.format({"Weight": "{:.2%}"}))
                st.markdown('</div>', unsafe_allow_html=True)
            ...
            with col_al2:
                ...
                # So sánh tỷ trọng giữa 3 chiến lược
                allo_merged = pd.merge(results_LSTM_GRU, Allo_1, on="Asset", suffixes=("_LSTM_GRU", "_Equal"))
                allo_merged = pd.merge(allo_merged, Allo_2, on="Asset")
                allo_merged.columns = ["Cổ phiếu", "LSTM-GRU (Mô hình)", "Phân bổ đều", "Phân bổ 80-20"]
                
                fig_alloc = go.Figure()
                fig_alloc.add_trace(go.Bar(x=allo_merged["Cổ phiếu"], y=allo_merged["LSTM-GRU (Mô hình)"]*100, name="LSTM-GRU (Mô hình)", marker_color='#3B82F6'))
                fig_alloc.add_trace(go.Bar(x=allo_merged["Cổ phiếu"], y=allo_merged["Phân bổ đều"]*100, name="Phân bổ đều", marker_color='#10B981'))
                fig_alloc.add_trace(go.Bar(x=allo_merged["Cổ phiếu"], y=allo_merged["Phân bổ 80-20"]*100, name="Phân bổ 80-20", marker_color='#F59E0B'))
            ...
            # Vẽ biểu đồ dạng khối (Treemap) tương tác của Plotly thay vì squarify tĩnh
            st.markdown('<div class="card-box">', unsafe_allow_html=True)
            ...
            st.info("Hãy thực hiện huấn luyện mô hình trước tại tab 'Huấn Luyện LSTM-GRU' để xem kết quả phân bổ tỷ trọng.")
        ...
        if st.session_state.get("trained", False):
            ...
            with col_kpi1:
                ...
                <div class="card-box">
                    ...
                    <div class="card-value">{max(sharpe_lstm, sharpe_1, sharpe_2):.4f}</div>
                    <div class="card-subtitle">Chiến lược tối ưu vượt trội</div>
                </div>
                """, unsafe_allow_html=True)
            with col_kpi2:
                st.markdown(f"""
                <div class="card-box">
                    <div class="card-title">Lợi Nhuận LSTM-GRU</div>
                    <div class="card-value">{Er_lstm:.2%}</div>
                    <div class="card-subtitle">Tập hợp phân bổ tối ưu động</div>
                </div>
                """, unsafe_allow_html=True)
            with col_kpi3:
                st.markdown(f"""
                <div class="card-box">
                    <div class="card-title">Rủi ro (Độ lệch chuẩn) LSTM-GRU</div>
                    <div class="card-value">{std_lstm:.2%}</div>
                    <div class="card-subtitle">Mức độ biến động năm</div>
                </div>
                """, unsafe_allow_html=True)
            ...
            st.info("Hãy thực hiện huấn luyện mô hình trước tại tab 'Huấn Luyện LSTM-GRU' để xem kết quả so sánh hiệu quả đầu tư.")
